...
if __name__ == '__main__':
    data = pd.read_csv("fifty1.csv")
    shape = int((data.shape[0] + 1) / 2)
    print("shape:", shape)
    percent=0
    with open('testplus.csv', mode='w') as csv_file:
        fieldnames = ['Position1', 'Position2', 'Position3', 'Position4', 'Position5', 'Position6','Prediction','Result']
        writer = csv.DictWriter(csv_file,fieldnames=fieldnames)
        for i in range(shape):
            print(i+1)
            positions=numpy.zeros(6)
            for j in range(6):
                positions[j] = float(data.iloc[2*i][j])
            demo = BlockTower(positions)
            result=demo.run_person()
            if(result[0]==result[1]):
                percent+=1
            writer.writeheader()
            writer.writerow(
            {'Position1': positions[0], 'Position2': positions[1], 'Position3': positions[2], 'Position4': positions[3],
             'Position5': positions[4], 'Position6': positions[5], 'Prediction': result[0], 'Result': result[1]})
    print(percent/shape)
    pygame.quit()